...
// constructor
    ...
    public void run() {
        ...
        while (true) {
            ...
            if ((buffer_index > (F0SIZE + F1SIZE)) && flag_enable_algo) {
                ...
                algo_index += 1;
                if (algo_index >= (BUFSIZE - (F0SIZE + F1SIZE))) {
                    algo_index = 0;
                }
            }
    ...
    private void send_udp_data(SensorEvent event){
        ...
        String dataline= Float.toString(event.values[0])+ ":"+ Float.toString(event.values[1])+":"+Float.toString(event.values[2]);
    ...
    private boolean detect_fall_phase_2( int start, int stop, double threshold){
        ...
        for( int i=start;i<stop;i+=1){
            ...
            y2=buffer_ay[i];
            z2= buffer_az[i];
            t= (Math.sqrt(Math.pow(Math.abs(x2),2)+ Math.pow(Math.abs(y2),2)+Math.pow(Math.abs(z2),2)))/ SensorManager.GRAVITY_EARTH;
            if(t> max) max=t;
            if(t< min) min=t;

        }
        if((max-min)> threshold){
            System.out.println("***********FALL P@ DETECTED************");
            return true;
